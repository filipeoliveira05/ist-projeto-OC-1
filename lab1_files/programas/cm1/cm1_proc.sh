...
ARRAY_SIZE=`cat cm1-3.out | cut -f1 | cut -d"=" -f2 |sort -gu`
STRIDE_LIST=`cat cm1-3.out | cut -f2 | cut -d"=" -f2 |sort -gu`
...
for stride in $STRIDE_LIST; do
    ...
    for cache in $ARRAY_SIZE; do
        METRIC=$(cat cm1-3.out | grep "cache_size=$cache	" | grep "stride=$stride	"  | cut -f3 | cut -d"=" -f2)
        if [[ -z $METRIC ]]; then
            METRIC=0.0;
        fi
